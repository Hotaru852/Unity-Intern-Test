...
public class Board
{
    ...
    private NormalItem.eNormalType[] allTypes;
    //
    ...
    public Board(Transform transform, GameSettings gameSettings)
    {
        ...
        //Nguyen Duy Quy
        queue = new Cell[5];
        allTypes = (NormalItem.eNormalType[])System.Enum.GetValues(typeof(NormalItem.eNormalType));
        //
    ...
    private void CreateBoard()
    {
        ...
        //Nguyen Duy Quy
        float boardCenterX = (boardSizeX - 1) * 0.5f;
        float startX = boardCenterX - 2f;
        float extraRowY = -2f;
        int cells = 5;
        ...
        for (int i = 0; i < cells; i++)
        {
            ...
            go.transform.position = origin + new Vector3(xPos, extraRowY, 0f);
            go.transform.SetParent(m_root);

            Cell cell = go.GetComponent<Cell>();

            cell.Setup((int)xPos, (int)extraRowY);
    ...
    //Nguyen Duy Quy
    internal void FillMultipleOf3()
    {
        int totalCells = boardSizeX * boardSizeY;
        int remainder = totalCells;
        int iType = 0;

        Dictionary<NormalItem.eNormalType, int> usageMap = new Dictionary<NormalItem.eNormalType, int>();
        ...
        while (remainder >= 3)
        {
            usageMap[allTypes[iType]] += 3;
            remainder -= 3;
            iType = (iType + 1) % allTypes.Length;
        }

        List<NormalItem.eNormalType> itemTypes = new List<NormalItem.eNormalType>(totalCells);
        ...
        for (int y = 0; y < boardSizeY; y++)
        {
            ...
            {
                Cell cell = m_cells[x, y];
                var chosenType = itemTypes[index++];
    ...
    //Nguyen Duy Quy
    public List<Cell> GetQueueMatches()
    {
        List<Cell> matchedCells = new List<Cell>();

        Dictionary<NormalItem.eNormalType, List<Cell>> typeToCells = new Dictionary<NormalItem.eNormalType, List<Cell>>();
        ...
        for (int i = 0; i < queue.Length; i++)
        {
            ...
            if (!typeToCells.ContainsKey(type))
            {
                typeToCells[type] = new List<Cell>();
            }

            typeToCells[type].Add(cell);
        }

        foreach (var entry in typeToCells)
        {
            if (entry.Value.Count >= 3)
            {
                matchedCells.AddRange(entry.Value);
            }
        }
    ...
    public Dictionary<NormalItem.eNormalType, List<Cell>> GetCellsGroupedByType()
    {
        ...
        foreach (Cell cell in allCells)
        {
            if (cell.Item != null)
            {
                ...
                if (nItem != null)
                {
                    if (!groups.ContainsKey(nItem.ItemType))
                        groups[nItem.ItemType] = new List<Cell>();
                    groups[nItem.ItemType].Add(cell);
                }
            }
        }
        return groups;
    ...
    //Nguyen Duy Quy
    internal void ShiftLeftQueueItems()
    {
        int shiftCount = 0;

        for (int i = 0; i < queue.Length; i++)
        {
            Cell currentCell = queue[i];
            ...
            else
            {
                if (shiftCount > 0)
                {
                    Cell targetCell = queue[i - shiftCount];
    ...
    internal void MoveItemToQueue(Cell sourceCell)
    {
        ...
        sourceCell.Free();
        itemToMove.OriginalCell = sourceCell;
        targetCell.Assign(itemToMove);

        itemToMove.View.DOMove(targetCell.transform.position, 0.3f);
    }

    internal void ReturnItemFromQueue(Cell queueCell)
    {
        if (queueCell.Item == null)
            return;

        Item item = queueCell.Item;
        ...
        item.View.DOMove(item.OriginalCell.transform.position, 0.3f);

        item.OriginalCell = null;
    }

    internal bool IsQueueCell(Cell cell)
    {
        for (int i = 0; i < queue.Length; i++)
        {
            if (queue[i] == cell)
